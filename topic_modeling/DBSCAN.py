...
def main():
    ...
    #k_fit = KMeans(n_clusters = number_of_clusters).fit_predict(cos_distance)
    k_fit = DBSCAN(eps = 0.555, min_samples = 3, metric = 'precomputed').fit_predict(cos_distance)
   
    number_of_clusters = len(set(k_fit))
    k_fit[k_fit==-1] = number_of_clusters
    figure(figsize=(15,15))
    x = np.arange(10)
    #yys = [i+x+(i*x)**2 for i in range(number_of_clusters)]
    colors = plt.cm.nipy_spectral(np.linspace(0, 1, number_of_clusters+1))
    #print colors[24]

    #plt.scatter(xs,ys, c=k_fit, s=100)
    for idx in range(0,number_of_clusters+1):
